...
struct CreateVehicleFormView: View {
    ...
    var body: some View {
        NavigationView {
            Form{
                Section {
                    ...
                    Picker("Välj tillverkare", selection: self.$createVehicleVM.manufacturor){
                        ForEach(self.manufacturorVehicleVM.manufacturors, id: \.id){ make in
                            Text(make.name).tag(make.name)
                        }
                    }.pickerStyle(.automatic)
                    TextField("Ange registeringsnummer", text: self.$createVehicleVM.registrationNumber)
            ...
            self.manufacturorVehicleVM.getAllManufacturors()
        }
    }
}
